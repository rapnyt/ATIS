...
if _sky_few != None:
  _sky_few_cov, _sky_few_quan = _sky_few.groups()
else:
  _sky_few_cov = None

if _sky_sct != None:
  _sky_sct_cov, _sky_sct_quan = _sky_sct.groups()
else:
  _sky_sct_cov = None

if _sky_bkn != None:
  _sky_bkn_cov, _sky_bkn_quan = _sky_bkn.groups()
else:
  _sky_bkn_cov = None

if _sky_ovc != None:
  _sky_ovc_cov, _sky_ovc_quan = _sky_ovc.groups()
else:
  _sky_ovc_cov = None
  ...
  if int(_vis_reg) == 9999:
    ...
    if int(_vis_reg) < 6000 and int(_vis_reg) >= 1000:
      if int(_vis_reg[1]) == 0:
        ...
        vis = "Visibility " + _vis_reg[0] + " thousands " + _vis_reg[1] + " hundret meters. "
    if int(_vis_reg) < 1000:
      vis = "Visibility " + _vis_reg[0] + " meters. "
    if _vis_dir != None and int(_vis_dir[1]) == 0 and len(_vis_dir) == 5 :
      vis += "Directional visibility " + _vis_dir[0] + " kilometers " + _vis_dir[4] + "."
    elif _vis_dir != None and int(_vis_dir[1]) != 0 and len(_vis_dir) == 5 :
      vis += "Directional visibility " + _vis_dir[0] + " kilometers " + _vis_dir[1] + " hundret meters " + _vis_dir[4] + "."
    elif _vis_dir != None and int(_vis_dir[1]) != 0 and len(_vis_dir) == 5 :
      vis += "Directional visibility " + _vis_dir[0] + " hundret meters " + _vis_dir[4] + "."
    if _vis_dir != None:
      vis = vis.replace("W","West") 
      vis = vis.replace("N","North")
      vis = vis.replace("E","East") 
      vis = vis.replace("S","South") 
    if _vis_rvr_rw != None and _vis_rvr_vis != None  and _vis_rvr_vis[1] != 0:
      vis += "Runway visual range, runway " + stretch(_vis_rvr_rw) + " . " + _vis_rvr_vis[0] + " thousand " + _vis_rvr_vis[1] + " hundret meters. " + _vis_rvr_vis[4] + ". "
    elif _vis_rvr_rw != None and _vis_rvr_vis != None  and _vis_rvr_vis[1] == 0:
      vis += "Runway visual range, runway " + stretch(_vis_rvr_rw) + " . " + _vis_rvr_vis[0] + " thousand meters. "+ _vis_rvr_vis[4] + ". "
    elif _vis_rvr_rw != None and _vis_rvr_vis != None  and _vis_rvr_vis[0] == 0:
      vis += "Runway visual range, runway " + stretch(_vis_rvr_rw) + " . " + _vis_rvr_vis[1] + " hundret meters. "+ _vis_rvr_vis[4] + ". "
    if _vis_rvr_rw != None:
      vis = vis.replace(" D."," decreasing ")
      vis = vis.replace(" N."," non-changing ")
      vis = vis.replace(" U."," raising ")

#Sky clouds or sky decoding decoding

if _sky_few_cov != None:
  if _sky_few_quan[1] != "0" and _sky_few_quan[2] != "0":
    sky += sky_cov[_sky_few_cov] + " at " + _sky_few_quan[1] + " thousand " + _sky_few_quan[2] + " hundret feet. "
  elif _sky_few_quan[2] == "0":
    sky += sky_cov[_sky_few_cov] +" at "+ _sky_few_quan[1] + " thousand feet. "
  elif _sky_few_quan[1] == "0":
    sky += sky_cov[_sky_few_cov] +" at "+ _sky_few_quan[2] + " hudret feet. "

if _sky_sct_cov != None:
  if _sky_sct_quan[1] != "0" and _sky_sct_quan[2] != "0":
    sky += sky_cov[_sky_sct_cov] +" at "+ _sky_sct_quan[1] + " thousand " + _sky_sct_quan[2] + " hundret feet. "
  elif _sky_sct_quan[2] == "0":
    sky += sky_cov[_sky_sct_cov] +" at "+ _sky_sct_quan[1] + " thousand feet. "
  elif _sky_sct_quan[1] == "0":
    sky += sky_cov[_sky_sct_cov] +" at "+ _sky_sct_quan[2] + " hudret feet. "

if _sky_bkn_cov != None:
  if _sky_bkn_quan[1] != "0" and _sky_bkn_quan[2] != "0":
    sky += sky_cov[_sky_bkn_cov] +" at "+ _sky_bkn_quan[1] + " thousand " + _sky_bkn_quan[2] + " hundret feet. "
  elif _sky_bkn_quan[2] == "0":
    sky += sky_cov[_sky_bkn_cov] +" at "+ _sky_bkn_quan[1] + " thousand feet. "
  elif _sky_bkn_quan[1] == "0":
    sky += sky_cov[_sky_bkn_cov] +" at "+ _sky_bkn_quan[2] + " hudret feet. "

if _sky_ovc_cov != None:
  if _sky_ovc_quan[1] != "0" and _sky_ovc_quan[2] != "0":
    sky += sky_cov[_sky_ovc_cov] +" at "+ _sky_ovc_quan[1] + " thousand " + _sky_ovc_quan[2] + " hundret feet. "
  elif _sky_ovc_quan[2] == "0":
    sky += sky_cov[_sky_ovc_cov] +" at "+ _sky_ovc_quan[1] + " thousand feet. "
  elif _sky_ovc_quan[1] == "0":
    sky += sky_cov[_sky_ovc_cov] +" at "+ _sky_ovc_quan[2] + " hudret feet. "

if _sky_vv != None:
  sky = "Vertical visibility " + _sky_vv[0] + " " + _sky_vv[1] + " thousand " + _sky_vv[2] + " hundret feet. "
  sky = sky.replace("0 0 thousand","")
  sky = sky.replace("0 hundret","")
