...
def metar_decoder(scrapped_metar):
    ...
    _wind = re.search(r"(\d{3})?(VRB)?(\d{2})KT(?:G(\d+))?(?:.+(\d{3})V(\d{3}))?", scrapped_metar)
    _vis = re.search(r"\s(\d{4})\s((?:\d{4}[NWSE]))?(?:R(\d{2}[LR]?)/(\d{4}[NDU]))?", scrapped_metar)
    _wx_det = re.findall(r"([-+]?[A-Z]{2,7})", scrapped_metar)
    _sky_few = re.search(r"(FEW)(\d{3})(CB)?(TCU)?", scrapped_metar)
    _sky_sct = re.search(r"(SCT)(\d{3})(CB)?(TCU)?", scrapped_metar)
    _sky_bkn = re.search(r"(BKN)(\d{3})(CB)?(TCU)?", scrapped_metar)
    _sky_ovc = re.search(r"(OVC)(\d{3})(CB)?(TCU)?", scrapped_metar)
    _sky_skc = re.search(r"SKC", scrapped_metar)
    _sky_nsc = re.search(r"NSC", scrapped_metar)
    _sky_vv = re.search(r"VV(\d{3})", scrapped_metar)
    _cavok = re.search(r"CAVOK", scrapped_metar)
    _temp_pres = re.search(r"(M?\d{2})/(M?\d{2}).Q(\d{3,4})", scrapped_metar)
    ...
    decoded_metar = wind + vis + sky + wx_final + temp_group + qnh

    return decoded_metar


def metar_scrapper():
    url = 'https://awiacja.imgw.pl/metarmil.php?airport=EPPW'
    page = urlopen(url)
    html_bytes = page.read()
    html = html_bytes.decode("utf-8")

    start_index = html.find("METAR EPPW")
    end_index = html.find("RMK")
    if html.find("NIL") >= 0:
        return False

    metar = html[start_index:end_index]
    # print(metar)
    return metar
...
metar2 = ""
# while True:
#     metar = metar_scrapper()
#     if metar is False:
#         print("Warning! Old Met info! ")
#         engine.say("Warning! Old Met info! ")
#         engine.runAndWait()
#         time.sleep(10)
#     if metar != metar2 and metar is not False:
#         try:
#             new_metar = metar_decoder(metar)
#             print("New METAR at", datetime.datetime.now())
#             print(new_metar)
#             engine.say(new_metar)
#             engine.runAndWait()
#         except:
#             print("METAR error.", metar, " at ", datetime.datetime.now())
#             engine.say("METAR error")
#             engine.runAndWait()
#             pass
#         metar2 = metar
    # time.sleep(60)
